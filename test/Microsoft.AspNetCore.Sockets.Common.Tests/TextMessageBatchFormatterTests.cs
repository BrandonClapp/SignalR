...
namespace Microsoft.AspNetCore.Sockets.Tests
{
    public class TextMessageBatchFormatterTests
    {
        ...
        {
            Assert.Equal(length, TextMessageBatchFormatter.MeasureMessages(messages));
        }
        ...
        public void WriteSingleTextMessage(string encoded, MessageType messageType, string payload)
        {
            ...
            var encodedLength = TextMessageBatchFormatter.MeasureMessages(messages);
            var buffer = new byte[encodedLength];

            Assert.True(TextMessageBatchFormatter.WriteMessages(buffer, messages));
        ...
        {
            var ex = Assert.Throws<FormatException>(() => TextMessageBatchFormatter.ReadMessages(Encoding.UTF8.GetBytes(encoded)).ToArray());
